...
把论文 PDF 提交给 Agent，按 34（工程型）/ 32（学术型）条 checklist 逐项审查（含 6 条
...
arXiv 预印本条目著录格式兼容 GB/T 7714 新旧国标）与硕士版完全对齐。
...
| 7 | 全量审查 | 其余约 30 条按"评判 + 定位 + 修改建议"模式逐条审查（含 6 条 UND-ENG-F 参考文献规范） |
...
### `references/engineering-30.md`（工程型，34 条）
...
| 参考文献 | `UND-ENG-F-*` | 6 | 双向引用抽样 / 编程书 + arXiv 上限 / 英文文献门槛 + 防伪 / [N] 编号规范 / 非同行评议来源处理 / arXiv 著录格式（7714 新旧国标兼容） |
...
ENG-F-01..F-06 的设计，仅在数量门槛上做本科化调整，其他规范要求与硕士版保持一致；
F-06 完全沿用硕士版的 arXiv / 预印本条目著录格式规则（[PP/OL] 推荐、[EB/OL] 兼容）。

### `references/academic-30.md`（学术型，32 条）
...
| 参考文献 | `UND-ENG-F-*` | 6 | 复用工程版，规范要求与硕士版完全一致（含 F-06 arXiv 著录格式） |
...
ML / NLP 方向的学术型本科论文常密集引用 arXiv 预印本，**F-06 命中概率显著高于工程型**，
但规则完全相同（兼容 GB/T 7714—2025 的 [PP/OL] 与 GB/T 7714—2015 的 [EB/OL]，均不报错）。

文件中前 27 条为"占位指引"，加载时回到 `engineering-30.md` 读取完整定义；仅 `UND-ACA-S-01..05`
